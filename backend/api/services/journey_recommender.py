...
def pick_best_triplet(
    ...
    """
    max_score, best = -1.0, (None, None, None) # khởi tạo để luôn có combo tốt hơn, none là best
    limit = total_budget * (1.0 + over_allow_ratio) # ngưỡng cho phép=budget + budget*0.1
    breakfasts = groups.get("breakfast", [])
    lunches = groups.get("lunch", [])
    dinners = groups.get("dinner", [])

    #brute-force thử tất cả kết hợp 
